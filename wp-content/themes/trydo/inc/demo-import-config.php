...
function trydo_after_import_setup($selected_import)
{
    ...
    //  Update URL 
    $rbt_options_old_url = get_option('trydo_options');
    $site_url = get_site_url();
    foreach($rbt_options_old_url as $key => $val) {
        if(isset($rbt_options_old_url[$key]['url'])) {
            if (str_contains($rbt_options_old_url[$key]['url'], 'http://rainbowit.net/themes/trydo')) {
                $rbt_options_old_url[$key]['url'] = str_replace('http://rainbowit.net/themes/trydo', $site_url, $rbt_options_old_url[$key]['url']);
            }
        }
    }
    update_option('trydo_options', $rbt_options_old_url); //update database
